...
/** Suspend reason codes (Postman: GEN_POL). */
...
export default function PolicySuspendPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error((data.error as string) ?? `Suspend failed (${res.status})`);
      }
      ...
      setSubmitError(e instanceof Error ? e.message : "Suspend failed.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <Card className="mb-6 border-border bg-card">
        <CardHeader>
          <CardTitle>Review suspend</CardTitle>
          <CardDescription>
            Confirm the policy and reason below. This will suspend the policy using the same API as the Monzo UAT Postman collection (PATCH /policy/policies/suspend).
          </CardDescription>
        </CardHeader>
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Suspend details</CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          {submitSuccess && (
            <p className="text-sm text-green-600 dark:text-green-400">Policy suspended successfully. Redirecting…</p>
          )}
