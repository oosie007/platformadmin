...
@UntilDestroy()
@Component({
  selector: 'canvas-event-variant-level',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './event-variant-level.component.html',
  styleUrls: ['./event-variant-level.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class EventVariantLevelComponent implements OnInit {
  ...
  eventTypes: MasterData[] = [];
  coverageVariantLevels: CoverageVariantLevel[];
  retrieveDataOptions: MasterData[] = [];
  selectedEventTypes: InsuredType[];
  eventParam: string;
  eventTypeToRenderIndex: number;
  eventTypeToRender: InsuredType | undefined;
        ...
        if (shouldPatch) {
          this._variantLevelService
            .patchCoverageVariantLevel(
              ...
              this._preparePatchRequestEvent(patchCoverageVariantLevelId ?? '')
            )
            .subscribe({
              next: (res) => {
                this._layoutService.showMessage({
  ...
  previous(): void {
    if (this.coverageVariantLevels.length != this.selectedEventTypes.length) {
      this._navigate(-1);
    } else {
      if (this.cvLevelId === '') {
  ...
  private _navigate(stepAdj?: number): void {
    const navEvent =
      this.selectedEventTypes[this.eventTypeToRenderIndex + (stepAdj || 0)];
    if (
      !isEmpty(navEvent) &&
      this.cvLevelId !== '' &&
      ...
      this._router.navigate(
        ...
        { queryParams: { key: navEvent.value } }
      );
    } else if (!isEmpty(navEvent)) {
      this._router.navigate(
      ...
      this._router.navigate(
        ...
        { queryParams: { key: this.eventParam } }
      );
    }
    this.onClear();
  }

  private _preparePatchRequestEvent(
    ...
    const insuredEventLevels: InsuredEventLevel[] = !isNullOrUndefined(
      covVariantLevel
    )
      ? covVariantLevel?.insuredEventLevel
      : [];

    const insuredLevel = this._prepareInsuredLevel();
    const miDataIndex = insuredEventLevels?.findIndex(
      (ele) => ele.insuredEventType.value === this.eventParam
    );
    if (miDataIndex > -1) {
      insuredEventLevels[miDataIndex] = insuredLevel;
    } else {
      insuredEventLevels.push(insuredLevel);
    }
    ...
    return {
      ...
      insuredEventLevel: insuredEventLevels
    };
  }

  private _preparePostRequestEvent(): PostCoverageVariantLevelRequest {
    const insuredLevels: InsuredEventLevel[] = [];
      ...
      coverageVariantLevels: [
        {
          ...
          insuredEventLevel: insuredLevels
        },
      ],
    };
  }

  private _prepareInsuredLevel(): InsuredEventLevel {
    ...
    return {
      insuredEventLevelId: crypto.randomUUID(),
      insuredEventType: this.selectedEventTypes
        .filter((ele) => ele.value === this.eventParam)
        ?.map((ele: messageKey) => {
          return {
            key: ele.key,
            value: this.eventParam,
 ...
 _observeAndSetValidators(): void {
    ...
    this.variantLevelForm?.get('limitsForm')?.get('maxLimitType')?.valueChanges?.subscribe((val: string) => {
    ...
    this.variantLevelForm?.get('limitsForm')?.get('aggregateLimitType')?.valueChanges?.subscribe((val: string) => {
        if (val && val.toLowerCase() === 'amount') {
          this.variantLevelForm
            .get('limitsForm')
    ...
    this.deductiblesForm?.get('valueType')?.valueChanges?.subscribe((val: string) => {
      if (val?.toUpperCase() === DeductibleValueTypes.PERCENTAGE) {
        this.deductiblesForm?.get('percentOf')?.setValidators(Validators.required);
      } else {
    ...
    combineLatest([
      ...
      next: (response) => {
        ...
        this.selectedEventTypes =
          response[5].insuredEvents?.map((obj) => obj.type) || [];
        this.eventTypeToRenderIndex = this.selectedEventTypes.findIndex(
          (obj) => obj.value === this.eventParam
        );
        if (
          isEmpty(this.selectedEventTypes) ||
          this.eventTypeToRenderIndex <= -1
        ) {
          this._layoutService.showMessage({
            severity: 'error',
            message: 'No Insured Events for this product.',
            duration: 5000,
          });
        }
        this.eventTypeToRender =
          this.selectedEventTypes[this.eventTypeToRenderIndex];
        ...
        this._initForm();
        // Observe and set validators after the form has been initialized
  ...
  private _prepareForPrefill(): void {
    this.retrieveDataOptions =
      ...
      this.coverageVariantLevels[0]?.insuredEventLevel?.map((ele) => {
        return {
          code: ele.insuredEventType?.value,
          description: ele.insuredEventType?.value,
        };
      });
    const insuredEventAvailable =
      !!this.retrieveDataOptions &&
      this.retrieveDataOptions.find((ele) => ele['code'] === this.eventParam);
    if (!isEmpty(insuredEventAvailable)) {
      this.retrieveDataControl.patchValue(insuredEventAvailable);
      this._prefillForm(insuredEventAvailable);
    }
  }
  ...
  private _prefillForm(selectedOption: MasterData | undefined) {
    ...
    const filteredVariantLevelData =
      coverageVariantLevel?.insuredEventLevel?.find(
        (ele) => ele.insuredEventType.value === selectedOption?.code
      );
    if (
