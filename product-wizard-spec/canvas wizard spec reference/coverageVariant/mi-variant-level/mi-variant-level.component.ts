...
import {
  ...
  PopUpModalConfig,
} from './model/mi-variant-level.model';
...
@UntilDestroy()
@Component({
  selector: 'canvas-mi-variant-level',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './mi-variant-level.component.html',
  styleUrls: ['./mi-variant-level.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class MiVariantLevelComponent implements OnInit {
  ...
  editSchema: UIKitNgxFormlyFormJsonSchema;
  ...
  constructor(
    ...
    this.editSchema = <UIKitNgxFormlyFormJsonSchema>(
      this._appContext.get('pages.product.mi-variant.editSchema')
    );
    ...
    this.labels = <MiVariantLevelLabels>(
      this._appContext.get('pages.product.mi-variant.labels')
    );
  ...
  _preparePatchRequestObjectWithFactors(
    ...
    const miDataIndex = insuredLevels.findIndex(
      (ele) => ele.insuredType?.value === InsuredTypeKeys.MAININSURED
    );
    ...
    combineLatest([
      ...
      next: (res) => {
        ...
        this.currentInsuredTypeIndex = this.selectedInsuredTypes.findIndex(
          (ins) => ins.value === InsuredTypeKeys.MAININSURED
        );
        this.insuredTypes = [...insuredTypes, ...dependentTypes];
        this.coverageVariantLevels = this.rootData.coverageVariantLevels;
  ...
  _initForm(): void {
    ...
    const subCoveragesForm = this._fb.group({
  ...
  removeSubCoverage(index: number): void {
    ...
    if (
      subCoverageLevelId &&
      this.retrieveDataControl?.value?.code === InsuredTypeKeys.MAININSURED
    ) {
      this.openModal = true;
    } else {
  ...
  getSelectedOption(): MasterData | undefined {
    let miData = this.retrieveDataOptions?.find(
      (ele) => ele['code'] === InsuredTypeKeys.MAININSURED
    );
  ...
  prefillRetrieveDataControl(coverageVariantLevelId: string | undefined): void {
    ...
    const miDataAvailable =
      !!this.retrieveDataOptions &&
      this.retrieveDataOptions.find(
        (ele) => ele['code'] === InsuredTypeKeys.MAININSURED
      );
    if (!isEmpty(miDataAvailable)) {
      this.showAddInfoAccordinContainer
        ? this.retrieveDataControl.patchValue(miDataAvailable)
        : this.newRetrieveDataControl.patchValue(miDataAvailable);
    }
  }
  ...
  It takes cvLevelId, coverageVariantLevels, and returns the description (or an empty string if not found)
  */
