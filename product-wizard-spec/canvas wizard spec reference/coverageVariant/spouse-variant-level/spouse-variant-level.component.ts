...
@UntilDestroy()
@Component({
  selector: 'canvas-spouse-variant-level',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormComponent,
  ...
  templateUrl: './spouse-variant-level.component.html',
  styleUrls: ['./spouse-variant-level.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class SpouseVariantLevelComponent implements OnInit {
  ...
  _preparePatchRequestObject(
    ...
    const spDataIndex = insuredLevels.findIndex(
      (ele) => ele.insuredType?.value === InsuredTypeKeys.SPOUSE
    );
    if (spDataIndex > -1) {
      insuredLevels[spDataIndex] = insuredLevel;
    } else {
    ...
    combineLatest([
      ...
      next: (res) => {
        ...
        this.currentInsuredTypeIndex = this.selectedInsuredTypes.findIndex(
          (ins) => ins.value === InsuredTypeKeys.SPOUSE
        );
  ...
  removeSubCoverage(index: number): void {
    ...
    if (
      subCoverageLevelId &&
      this.retrieveDataControl?.value?.code === InsuredTypeKeys.SPOUSE
    ) {
      this.openModal = true;
    } else {
  ...
  getSelectedOption(): MasterData | undefined {
    let miData = this.retrieveDataOptions?.find(
      (ele) => ele['code'] === InsuredTypeKeys.SPOUSE
    );
  ...
  prefillRetrieveDataControl(coverageVariantLevelId: string | undefined): void {
    ...
    const spDataAvailable =
      !!this.retrieveDataOptions &&
      this.retrieveDataOptions.find(
        (ele) => ele['code'] === InsuredTypeKeys.SPOUSE
      );
    if (!isEmpty(spDataAvailable)) {
      this.showAddInfoAccordinContainer
        ? this.retrieveDataControl.patchValue(spDataAvailable)
        : this.newRetrieveDataControl.patchValue(spDataAvailable);
    }
  }
