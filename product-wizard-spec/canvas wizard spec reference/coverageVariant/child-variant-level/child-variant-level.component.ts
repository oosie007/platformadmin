...
@UntilDestroy()
@Component({
  selector: 'canvas-child-variant-level',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './child-variant-level.component.html',
  styleUrls: ['./child-variant-level.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class ChildVariantLevelComponent implements OnInit {
  ...
  _preparePatchRequestObject(
    ...
    const childDataIndex = insuredLevels.findIndex(
      (ele) => ele.insuredType?.value === DependentTypeKeys.CHILD
    );
    if (childDataIndex > -1) {
      insuredLevels[childDataIndex] = insuredLevel;
    } else {
    ...
    combineLatest([
      ...
      next: (res) => {
        ...
        this.currentInsuredTypeIndex = this.selectedInsuredTypes.findIndex(
          (ins) => ins.value === DependentTypeKeys.CHILD
        );
  ...
  removeSubCoverage(index: number): void {
    ...
    if (
      subCoverageLevelId &&
      this.retrieveDataControl?.value?.code === DependentTypeKeys.CHILD
    ) {
      this.openModal = true;
    } else {
  ...
  getSelectedOption(): MasterData | undefined {
    let miData = this.retrieveDataOptions?.find(
      (ele) => ele['code'] === DependentTypeKeys.CHILD
    );
